 ...
 * @author magica
 */
public class IntersectingAlternativesMultipleExperiment2 extends Experiment {

    private int cycleLength = 1000;
    ...
    protected SchedulingResultsStats[] bsRealStats;
    ...
    protected void configureExperiment() {
        bsRealStats = new SchedulingResultsStats[limits.length];
        ...
        for(int i = 0; i < limits.length; i++){
            bsRealStats[i] = new SchedulingResultsStats();
    ...
    protected void performExperiment() {
        ...
        for (int iLimit = 0; iLimit  < limits.length; iLimit++) {
            ...
            ArrayList<UserJob> batchBSReal = VOEHelper.copyJobBatchList(batch);
            ArrayList<UserJob> batchBSAll = VOEHelper.copyJobBatchList(batch);

            flush();

            /* BS Really Sliced*/
            System.out.println("BS Sliced scheduling");
            bss.shiftAlternatives = true;
            bss.findAllPossibleAlternativesForJob = false;
            bss.slicesNum = 5;
            bs.solve(bss, env, batchBSReal);
            if (!SchedulingResultsStats.checkBatchForSuccess(batchBSReal)) {
                System.out.println("BS Skiced failed to find alternatives");
                bsRealStats[iLimit].addFailExperiment();
                success = false;
            }
            ...
            if (success) {
                ...
                SchedulerOperations.rateBatchAlternativesByBatch(batchBSReal, batchBSAll);
                ...
                bsRealStats[iLimit].processResults(batchBSReal);
    ...
    public String getData() {
        ...
        for (int i = 0; i < limits.length; i++) {
            ...
            data += "BS STATS with LIMIT = " + limits[i] + "\n"
                    ...
                    + "BS Sliced STATS with LIMIT = " + limits[i] + "\n"
                    + this.bsRealStats[i].getData() + "\n"
