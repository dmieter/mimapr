...
public class ResourceLineChart extends javax.swing.JPanel {
    ...
    private ArrayList<ComputingResourceLine> cpus;
    ...
    protected void myInit(){
        ...
        cpus = new ArrayList<ComputingResourceLine>();
    ...
    private int GetLastTaskEnd(){
        ...
        Slot s;
        DistributedTask t;
        for(ComputingResourceLine rl : cpus){
            if(rl.slots.size()>0){
...
//        }
    ...
    private void DrawCpuLine(ComputingResourceLine cpu, int vOffset, Graphics g){
        Graphics2D g2 = (Graphics2D)g;
        ...
        g2.drawString(cpu.name + " " + String.format("%.2f", cpu.q), cpunamepos, vOffset-rowheight/2.2f);

        g2.setFont(new Font("SansSerif", Font.BOLD,12));        //Price + Speed
        g2.drawString("Speed: "+ cpu.resourceType.getPerformance(), paramspos, vOffset - 2*rowdelta);
    ...
    public void SetVOEnvironment(VOEnvironment env){
        ComputingResourceLine cpu;
        cpus = new ArrayList<ComputingResourceLine>();       //erasing
        for (ComputingResourceLine rl : env.resourceLines){
            cpu = new ComputingResourceLine(rl);
            cpus.add(cpu);
        }
