...
public class SlotProcessor {
    ...
    protected ArrayList<Slot> getSortedSlots(VOEnvironment environment, SlotProcessorSettings settings) {
        ...
        for (ComputingResourceLine rLine : environment.resourceLines) {
    ...
    protected boolean checkSlotForWindow(Slot slot, Window window, SlotProcessorSettings settings) {
        if (settings.algorithmType.equals("NORMAL")) {
            return slot.getLength() >= slot.getVolumeTime(window.resourceRequest.volume)
                    && slot.getPrice() <= window.resourceRequest.maxNodePrice
                    && slot.getPerformance() >= window.resourceRequest.minNodePerformance;
        }
    ...
    protected boolean checkCostForWindow(Window window) {
        if (window.slots.size() < window.resourceRequest.resourceNeed) {
            return false;
        }
        //сортировка слотов по цене - побочное действие всей функции
        window.sortSlotsByCost();
        double s = 0;
        for (int i = 0; i < window.resourceRequest.resourceNeed; i++) {
            s += window.slots.get(i).getVolumeCost(window.resourceRequest.volume);
        }
        return s <= window.resourceRequest.getMaxCost();
    }
    ...
    protected Window FindNextWindow(ResourceRequest rt, ArrayList<Slot> slots, SlotProcessorSettings settings) {
        ...
        for (int i = 0; i < slots.size(); i++) {
            ...
            if (checkSlotForWindow(slot, window, settings)) {
                ...
                while (j < window.slots.size()) {
                    ...
                    if (checkedSlot.end - _start < checkedSlot.getVolumeTime(window.resourceRequest.volume)) {
            ...
            if ((window.slots.size() >= window.resourceRequest.resourceNeed) && settings.algorithmType.equals("NORMAL")
                    || settings.algorithmType.equals("MODIFIED") && checkCostForWindow(window)) {
                break;
            }
        }
        //в случае мод. алгоритма отрезаем лишние слоты
        if (settings.algorithmType.equals("MODIFIED") && window.slots.size() > window.resourceRequest.resourceNeed) {
            //window.slots = (ArrayList<Slot>)window.slots.subList(0,window.slotsNeed);
            window.slots = takeSublist(window.slots, window.resourceRequest.resourceNeed);
        }
        ...
        if (window.slots.size() < window.resourceRequest.resourceNeed
                || settings.algorithmType.equals("MODIFIED") && window.getTotalVolumeCost() > window.resourceRequest.getMaxCost()) {
            window = null;
        }
        return window;
    }
    ...
    protected void prettyCutWindow(Window w) {
        ...
        if (w != null) {
            w.slots = takeSublist(w.slots, w.resourceRequest.resourceNeed);     //getting the needed count of slots (first slotsneed slots will represent the best combination)
            //установка старта окна
        ...
        for (Slot s : w.slots) {
            s.start = w.startTime;
            double perfomanceTime = s.getVolumeTime(w.resourceRequest.volume);
            s.end = w.startTime + perfomanceTime;
