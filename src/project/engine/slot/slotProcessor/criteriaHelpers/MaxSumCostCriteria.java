...
public class MaxSumCostCriteria implements ICriteriaHelper {
    ...
    public double getCriteriaValue(Window w) {
        ...
        if (w.squareWindow) {
            throw new UnsupportedOperationException("MaxSumCostCriteria doesn't support square windows");
        }

        double sum = 0;

        if (w.resourceRequest.resourceNeed < w.slots.size()) {
            w.sortSlotsByCost();
    ...
    protected List<Slot> getExtraSlots(Window w) {
        ...
        for (Iterator<Slot> it = w.slots.iterator(); it.hasNext();) {
            Slot s = it.next();
            if (i >= w.resourceRequest.resourceNeed) {
                extraSlots.add(s);
                it.remove();
            }
            i++;
    ...
    protected boolean tryToMakeChange(Window w, List<Slot> extraSlots) {
        ...
        Slot cheapestSlot = w.slots.get(0);
        /* as slots sorted by cost */
        ...
        for (Iterator<Slot> it = extraSlots.iterator(); it.hasNext();) {
            ...
            if (currentWindowCost - cheapestCost + expensiveSlot.getVolumeCost(w) <= w.resourceRequest.getMaxCost()) {
