     ...
     double extremum;
            ...
            while(!backfillRequests.isEmpty())
                  {int csize = backfillRequests.size();                 
                   extremum = backfillRequests.get(0).resourceRequest.volume;                    
                   num = 0;
                  
                  
                     for(int i=0; i<csize; i++)  
                         if((extremum > backfillRequests.get(i).resourceRequest.volume && "DURATIONMIN".equals(settings.priorityPolicy)) 
                                 || (extremum < backfillRequests.get(i).resourceRequest.volume && "DURATIONMAX".equals(settings.priorityPolicy))) 
                         {extremum = backfillRequests.get(i).resourceRequest.volume; num = i;}
