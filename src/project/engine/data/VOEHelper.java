...
public class VOEHelper {
    ...
    public static ArrayList<Slot> getSlotsFromVOE(VOEnvironment voe, double start, double end) {
        ...
        for (Iterator it = copyEnv.resourceLines.iterator(); it.hasNext();) {
            ComputingResourceLine rl = (ComputingResourceLine) it.next();
    ...
    public static void trimSlots(VOEnvironment voe, double start, double end) {      //trims slot lists to the borders (works best after update slots or same kind)
        for (ComputingResourceLine rl : voe.resourceLines) {
    ...
    private static void updateResourceLineSlots(ComputingResourceLine rl) {
    ...
    private static void trimResourceLineSlots(ComputingResourceLine rl, double start, double end) {
    ...
    public static VOEnvironment copyEnvironment(VOEnvironment etalon) {
        ...
        newVOE.resourceLines = new ArrayList<ComputingResourceLine>();
        for (Iterator it = etalon.resourceLines.iterator(); it.hasNext();) {
            ComputingResourceLine rl = new ComputingResourceLine((ComputingResourceLine) it.next());
    ...
    private static boolean checkSlotIntersectionWithVOE(Slot s, VOEnvironment env) {
        ComputingResourceLine slotline = null;
        for (ComputingResourceLine line : env.resourceLines) {
    ...
    public static ComputingResourceLine getResourceLineByID(VOEnvironment voe, int resourceID) {
        for (ComputingResourceLine line : voe.resourceLines) {
    ...
    public static boolean isJobRunsOnResource(UserJob job, ComputingResourceLine resource, int time) {
        if (job == null || job.bestAlternative == -1) {
            return false;
            /* seems job isn't running */
        }
    ...
    public static DistributedTask getPreviousTask(int resourceLineID, double time, VOEnvironment env) {
        ComputingResourceLine rl = getResourceLine(env, resourceLineID);
        if (rl == null) {
            throw new IllegalStateException("Can't find Resource Line with ID=" + resourceLineID);
        }

        DistributedTask prevTask = null;

        for (DistributedTask task : rl.tasks) {
    ...
    /* returns either previous task end or schedulingInterval start */
    public static Double getPreviousEventTime(int resourceLineID, double time, VOEnvironment env) {
        
        DistributedTask prevTask = getPreviousTask(resourceLineID, time, env);
        
        if(prevTask != null){
            return Math.max(prevTask.endTime, env.intervalStartTime);
        }else{
            return env.intervalStartTime;
        }
    }

    public static DistributedTask getNextTask(int resourceLineID, double time, VOEnvironment env) {
        ComputingResourceLine rl = getResourceLine(env, resourceLineID);
        if (rl == null) {
            throw new IllegalStateException("Can't find Resource Line with ID=" + resourceLineID);
        }
    ...
    /* returns either previous task end or schedulingInterval start */
    public static Double getNextEventTime(int resourceLineID, double time, VOEnvironment env) {
        
        DistributedTask nextTask = getNextTask(resourceLineID, time, env);
        
        if(nextTask != null){
            return Math.min(nextTask.startTime, env.intervalEndTime);
        }else{
            return env.intervalEndTime;
        }
    }

    protected static ComputingResourceLine getResourceLine(VOEnvironment env, int resourceLineID) {
        for (ComputingResourceLine rl : env.resourceLines) {
            if (rl.id == resourceLineID) {
                return rl;
            }
        }
