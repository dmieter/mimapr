...
/**
 * Created by IntelliJ IDEA. User: Rookie Date: 07.04.2010 Time: 2:00:42 To
 * change this template use File | Settings | File Templates.
 */
public class VOEnvironment implements Serializable {

    public ArrayList<ComputingResourceLine> resourceLines;
    public EnvironmentPricingSettings pcSettings;
    
    public double intervalStartTime = 0;
    public double intervalEndTime = Double.POSITIVE_INFINITY;

    public VOEnvironment() {
        resourceLines = new ArrayList<ComputingResourceLine>();
    }

    public double getTotalOccupancy() {
        double s = 0;
        for (ComputingResourceLine rLine : resourceLines) {
        ...
        for (ComputingResourceLine rline : resourceLines) {
            ...
            if (pcSettings != null && pcSettings.speedLevels != null && pcSettings.speedLevels.length > 1
                ...
                for (int i = pcSettings.speedLevels.length - 1; i >= 0; i--) {
                    if (rline.getPerformance() >= pcSettings.speedLevels[i]) {
                        mutationPriceCoef = pcSettings.speedPriceQuotients[i];
                        rline.price = rline.getPerformance() * mutationPriceCoef;
                        break;
                    }
                }
            } else {
                ...
                rline.price = rline.getPerformance() * Math.exp(pcSettings.speedExtraCharge * (rline.getPerformance() - 1));
